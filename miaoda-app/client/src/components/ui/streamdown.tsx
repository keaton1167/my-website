...
import { isAttachmentFileUrl, parseAttachmentInfo } from '@shared/api.interface';

const LazyAttachmentPreview = lazy(() =>
  import('@client/src/components/business-ui/attachment-preview').then((m) => ({
...
> = {
  ...
  a: ({ className, href, children, ...props }) => {
    ...
    if (href && !isIncomplete && isAttachmentFileUrl(href)) {
      const childText = extractTextContent(children);
      const info = parseAttachmentInfo(href, childText);
      return (
        <Suspense
          fallback={
            <a
        ...
        >
          <LazyAttachmentPreview type={info.type} url={info.fileUrl} fileName={info.fileName} />
        </Suspense>
      );
    }
